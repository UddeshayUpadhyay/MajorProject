      ...
      String name=request.getParameter("name");
		...
		if(item!=null) {
			...
			<div class="row bg-light p-4 my-2">
				...
				<input type="hidden" name="name" value="<%=name %>">
				<input class="p-2" type="number" name="qty" value="<%=item.get("qty")%>"   required/>
				<button class="btn btn-sm btn-dark p-2">Update Quantity</button>
				</form>
					</div>
					<div class=" my-2 p-2 bg-secondary text-white">
					<h6 class="text-white">Update Item Details:</h6>
					<form action="UpdateItemDetails" method="post" >
				<input type="hidden" name="name" value="<%=name %>">
				Name:<input class="my-2" type="text" name="newname" maxlength="100" value="<%=item.get("name")%>"   required/><br>
				Comapany Name:<input class="my-2" type="text" name="cname" maxlength="100" value="<%=item.get("cname")%>"   required/><br>
				Salt(If Applicable)<input class="my-2" type="text" name="salt" maxlength="100" value="<%=item.get("salt")%>"   /><br>
				Price:<input class="my-2" type="text" name="price" maxlength="10" pattern="[0-9.]+" value="<%=item.get("price")%>"   required/><br>
				<button class="btn btn-sm btn-dark p-2 my-2">Update Details</button>
				</form>
					</div>
				</div>
			</div>
